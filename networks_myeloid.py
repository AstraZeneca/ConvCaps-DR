...
def CapsNet_DR(input_shape, args):
    ...
    x = conv_caps_DR(iCaps=args.cap_numbCap[4], iPose=args.cap_sizeCap[4], 
                     ...
                     batch=args.batch_size, name='Class_caps')(x)
...
# CapsNets-EM
# ----------------------------------------------------------------------------
""" A basic CapsNets with EM routing and 5 capsule-layers is defined below. 
Our experiments showed that the EM algorithm has more problems to converge. 
"""


from convcaps_em import primary_caps_EM, conv_caps_EM

def CapsNetEM(input_shape, args):
    """ A Capsule Network on with EM routing.
    # Arguments
        input_shape:    data shape, 3d, [width, height, channels]
        args:           A namespace with the parameters of the network
    ...
    # Capsusle section --------------------------------------------------------
    pose, actv = primary_caps_EM(oCaps=args.cap_numbCap[0], 
                                 oPose=args.cap_sizeCap[0], 
                                 k=args.cap_Kvalues[0], 
                                 strides=args.cap_strides[0], 
                                 do_poses=True,
                                 act_type='default')(x)
    print('Output Primary-Caps:   ', pose.shape)
        
    
    pose, actv = conv_caps_EM(iCaps=args.cap_numbCap[0], 
                              iPose=args.cap_sizeCap[0], 
                              oCaps=args.cap_numbCap[1], 
                              oPose=args.cap_sizeCap[1], 
                              k=args.cap_Kvalues[1], 
                              strides=args.cap_strides[1], 
                              iters=args.cap_routing, 
                              batch=args.batch_size, 
                              conv_cap=True)(pose, actv)
    print('Output Mid1-Caps:   ', pose.shape)
 
    
    pose, actv = conv_caps_EM(iCaps=args.cap_numbCap[1], 
                              iPose=args.cap_sizeCap[1], 
                              oCaps=args.cap_numbCap[2], 
                              oPose=args.cap_sizeCap[2], 
                              k=args.cap_Kvalues[2], 
                              strides=args.cap_strides[2], 
                              iters=args.cap_routing, 
                              batch=args.batch_size, 
                              conv_cap=True)(pose, actv)
    print('Output Mid2-Caps:   ', pose.shape)
 
 
    pose, actv = conv_caps_EM(iCaps=args.cap_numbCap[2], 
                              iPose=args.cap_sizeCap[2], 
                              oCaps=args.cap_numbCap[3], 
                              oPose=args.cap_sizeCap[3], 
                              k=args.cap_Kvalues[3], 
                              strides=args.cap_strides[3], 
                              iters=args.cap_routing, 
                              batch=args.batch_size, 
                              conv_cap=True)(pose, actv)
    print('Output Mid3-Caps:   ', pose.shape)
    
 
    # Class capsules ----------------------------------------------------------
    a_out = conv_caps_EM(iCaps=args.cap_numbCap[3], 
                         iPose=args.cap_sizeCap[3], 
                         oCaps=args.cap_numbCap[4], 
                         oPose=args.cap_sizeCap[4], 
                         k=args.cap_Kvalues[4], 
                         strides=args.cap_strides[4], 
                         iters=args.cap_routing,
                         batch=args.batch_size, 
                         conv_cap=False, w_shared=True, 
                         last_cap=True, coor_add=False)(pose, actv)
    print('Class capsules, activations:', a_out.shape)

    # Compile the model 
    model = Model(inputs=inputs, outputs=a_out, name='CapsNets_EM')
    nadam = Nadam(lr=args.lr_init, beta_1=0.9, beta_2=0.999, epsilon=1e-07)
    model.compile(optimizer=nadam,
                  loss=margin_loss,
                  metrics='categorical_accuracy')
